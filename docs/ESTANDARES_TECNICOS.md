...
**Mobiliario que no debería superponerse entre sí:** cama/escritorio/decoración ahora comparten un balde de ocupación (`FURNITURE_CATEGORIES` en `build_system.gd`) en vez de uno por categoría — antes un escritorio y una cama podían coexistir en el mismo punto exacto sin que el sistema lo notara, porque cada categoría tenía su propio diccionario aislado.
