...
- **Arreglo definitivo:** la orientación se decide mirando si **ya hay un piso construido** de un lado de la pared o del otro (`_cell_has_floor`, una consulta física corta en el centro de cada celda vecina). La cara linda siempre apunta lejos del piso interior. Esto es 100% relativo al mundo/piso ya construido, no al jugador — no cambia al caminar. Si todavía no hay piso de ningún lado (caso "sin contexto"), usa una orientación por defecto.
