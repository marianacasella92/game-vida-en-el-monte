...
- **Bug adicional encontrado (este/oeste invertido):** con paredes en escuadra, el eje norte/sur quedaba bien pero el este/oeste quedaba invertido. Se verificó con la matriz de rotación real (no a ojo): a `rot=-90°` la cara decorativa (eje local Z) en realidad apunta hacia **oeste**, no hacia el este como se había asumido al escribir el código por primera vez (esa suposición nunca se validó con la cuenta, solo con que la posición cerrara bien). Arreglado invirtiendo qué par (rotación, esquina de apoyo) corresponde a `face_east=true` vs `false` en la rama este/oeste — la rama norte/sur ya estaba bien.
- **El problema real (confirmado con datos, no a ojo):** con logs de posición/rotación exactos, se vio que *todas* las paredes del lado negativo de su eje (oeste, y el equivalente en Z) quedaban mal, y todas las del lado positivo bien — siempre la misma dirección, sin importar el lado real de la habitación. Eso apuntaba a que `_cell_has_floor` (que usaba una consulta de física con `SphereShape3D`) nunca detectaba el piso correctamente y siempre caía en el valor por defecto.
- **Arreglo final:** se sacó la consulta de física por completo. Ahora `BuildSystem` mantiene su propio registro (`floor_cells`, un diccionario de celda → true) de dónde puso piso, actualizado directamente en `_place_piece()`/`_remove_piece()`. La decisión de hacia dónde mira la pared consulta ese registro en vez de preguntarle a la física — mismo resultado conceptual, pero sin las capas de física/colisión que resultaron poco confiables.
