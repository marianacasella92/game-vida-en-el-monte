...
- [ ] Animar el ícono de vida (corazón): hoy es un sprite único (corazón + marco circular juntos). Separar en dos capas — el marco circular queda fijo, y el corazón se "recorta"/vacía de arriba hacia abajo según el % de vida actual (`TextureProgressBar` con `fill_mode` vertical, o un shader de clip). Corazón lleno = 100% de vida, corazón vacío = 0% = el momento del desmayo.
- [ ] Implementar el efecto de post-procesado ya documentado en `docs/GameDesign/PXD_Diseno_HUD_UI_v1.md` sección 2.4 (viñeta oscura + desenfoque radial en los bordes de pantalla + audio de respiración pesada) cuando la vida está bajando — quedó especificado en el diseño ("shader sobre un `ColorRect` en un `CanvasLayer`, activado por código según el valor") pero nunca se implementó. Va junto con el debuff de movimiento que ya existe (`neglected_speed_multiplier` en `player_needs.gd`) — moverse más lento + ver borroso, ambos a la vez.
- [ ] Ajustar `grant_death_grace()` (`autoload/player_needs.gd`): hoy pone hambre/sueño con un piso del 40% y vida al 100%. El comportamiento correcto es distinto — el hambre queda **tal cual estaba** en el último guardado (sin piso mínimo), el sueño se restaura al 100%, y la vida se restaura solo al **60%** (no al máximo).
- [ ] Al despertar del desmayo, aparecer al lado de la cama (no en la posición del último guardado tal cual) — necesita lógica de colisión/slot para no aparecer superpuesto con otra pieza. Depende de resolver antes el tamaño/slot de la cama (ítem de arriba, "La cama ocupa 3x1...") para tener un lugar de aparición confiable al lado.
