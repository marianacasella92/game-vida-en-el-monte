...
**Deuda técnica (resuelta 07/07/2026):**
- [x] Desacoplar la lógica de `build_system.gd`: `_unhandled_input`/`_process` tenían cadenas de `if/elif equipped_category == "..."` creciendo con cada categoría nueva, más dos listas paralelas (`ROTATABLE_CATEGORIES`/`FURNITURE_CATEGORIES`) que ya se habían desincronizado una vez (bug real: la cama no rotaba). **Resuelto:** una sola tabla `CATEGORY_BEHAVIOR` (`category -> {targeting, snap, rotatable, furniture}`, ver `docs/ESTANDARES_TECNICOS.md` "Categorías por comportamiento") — agregar una categoría nueva que se posiciona plana es sumar una entrada a esa tabla, no tocar `_unhandled_input()`/`_process()`. `"wall"` queda como caso especial explícito (auto-encaje según lo construido al lado), no forzado a la misma forma. No se usó una jerarquía de clases/recursos por categoría (más pesado de lo que hace falta hoy) — se mantuvo el criterio data-driven que ya usa `CropManager._state_actions`.
...
- [x] Realismo de sol y luna (feedback de la usuaria: "no se ven para nada reales"): el sol pasó de disco nítido a **resplandor** (núcleo 1.5° + halo ancho vía `sun_angle_max`/`sun_curve` + **Glow activado** en el Environment); la luna ahora es la **foto real** (`assets/enviroment/moon.png`) en un billboard lejano (`MoonPivot/MoonSprite` en `world.tscn`, QuadMesh 70u a 800u, blend ADD para que el fondo negro desaparezca) que rota con la noche — el disco procedural de la MoonLight se apagó (`angular_distance = 0`) para no dibujar dos lunas. `F5` ahora se puede **mantener apretado** para avance continuo del reloj (~1 día/segundo). **Pendiente estético siguiente: estrellas** (necesita panorama de cielo estrellado CC0, asset de la usuaria + shader de cielo custom — anotado, no urgente).
...
- [x] El autoguardado pisó una construcción en progreso durante una sesión de juego — piezas recién colocadas se perdieron/revirtieron. **No era el autoguardado:** `save_game()` solo escribe a disco, nunca toca el mundo. El culpable real es GDD 4.8 (muerte por hambre/sueño descuidado) — `PlayerNeeds.died` → `SaveManager._on_player_died() → load_game()`, que sí recarga piezas/inventario/plata al último guardado, **sin ningún aviso visual**. Absorta construyendo, la vida bajaba a 0 por descuido y el juego recargaba en silencio — se sentía como un bug al azar pisando el progreso. **Resuelto:** `hud.gd` ahora muestra un mensaje ("Te desmayaste... se recargó tu última partida guardada") por 4s cuando se dispara `PlayerNeeds.died`, para que la recarga deje de sentirse como un bug fantasma.
- [x] La cama atraviesa paredes al colocarla, y el fantasma se queda en verde (válido) aunque esté metida en la pared. Bounding box real de `Bed Single.glb`: ~2m x 1.35m x 3.9m — la caja de colisión estaba a mano en 1.1x1.0x2.1, menos de la mitad. **Resuelto en dos pasos:** (1) se corrigió la caja de colisión (`bed.tscn`) al tamaño real, y mobiliario (cama/escritorio/decoración) pasó a compartir un balde de ocupación común (`FURNITURE_CATEGORIES`) para no superponerse entre sí; (2) **el bug de fondo** era que nada chequeaba si el mueble se metía en una pared — `_slot_taken()` solo reserva un punto por categoría, no la forma real. Se agregó `_overlaps_wall()`: choca la colisión real del fantasma contra las paredes ya construidas específicamente para mobiliario, y ahora si se solapa con una pared el fantasma se marca inválido (rojo) en vez de dejar colocarlo. Se descartó (a propósito, por complejidad innecesaria) un sistema de "huella multi-celda" que reservaba celdas de grilla — el chequeo contra paredes con la colisión real resuelve el problema reportado sin esa complejidad. **Ajuste fino:** ese chequeo era demasiado estricto al principio — cualquier mueble apoyado contra una pared (no solo la cama) se invalidaba con solo tocarla, porque el espesor de la pared (0.4m) siempre mete la mitad hacia la celda vecina. Se agregó `WALL_OVERLAP_MARGIN` (tolera hasta el espesor de una pared) para que "apoyado justo contra la pared" sea válido, pero una intrusión de verdad se siga marcando inválida.

### Muerte/desmayo (GDD 4.8) — mejoras pendientes
Encontradas revisando el aviso de desmayo recién agregado (`hud.gd::_on_player_died`) — la dinámica de fondo (`autoload/player_needs.gd`) necesita varios ajustes más para sentirse completa.

- [x] Animar el ícono de vida (corazón): hoy es un sprite único (corazón + marco circular juntos). Separar en dos capas — el marco circular queda fijo, y el corazón se "recorta"/vacía de arriba hacia abajo según el % de vida actual. **Resuelto:** la usuaria separó el asset en `icon_health_heart.png` + `icon_health_frame.png` (mismo lienzo); en `player.tscn`, `HealthIcon` pasó de `TextureRect` a un `Control` con dos capas — `HeartFill` (`TextureProgressBar`, `fill_mode` BOTTOM_TO_TOP + `nine_patch_stretch` para escalar al tamaño del HUD) abajo y `Frame` fijo encima. `hud.gd::_update_health()` actualiza corazón y viñeta juntos desde `PlayerNeeds.health_changed`. El `icon_health.png` original queda sin uso (no se borra por si hace falta re-cortar).
- [x] (parcial) Implementar el efecto de post-procesado ya documentado en `docs/GameDesign/PXD_Diseno_HUD_UI_v1.md` sección 2.4 (viñeta oscura + desenfoque radial en los bordes de pantalla + audio de respiración pesada) cuando la vida está bajando. **Resuelto (visual):** `scenes/ui/low_health_vignette.gdshader` (mismo patrón 9-tap ya probado en Compatibility que `background_blur.gdshader`, pero con viñeta radial — centro despejado, bordes oscuros y borrosos) sobre un `ColorRect` en el `Hud` (`LowHealthVignette`, primer hijo para dibujarse detrás de las barras). `hud.gd` maneja la intensidad: 0 con vida ≥50% (`VIGNETTE_HEALTH_THRESHOLD`), crece linealmente hasta 1 al llegar a 0. Se oculta con pantallas modales abiertas (el Hud se dibuja encima de esas capas). **Audio de respiración pesada: resuelto** — la usuaria consiguió `breath.flac` (Godot 4 no soporta FLAC; convertido a `assets/fx/breath.ogg` con ffmpeg). `AudioStreamPlayer` "BreathAudio" en el Hud, en loop, arranca junto con la viñeta y sube de volumen a medida que baja la vida (`hud.gd::_update_health()`).
- [x] Ajustar `grant_death_grace()` (`autoload/player_needs.gd`): hoy pone hambre/sueño con un piso del 40% y vida al 100%. El comportamiento correcto es distinto — el hambre queda **tal cual estaba** en el último guardado (sin piso mínimo), el sueño se restaura al 100%, y la vida se restaura solo al **60%** (no al máximo). **Resuelto:** hambre ya no se toca (queda como la dejó `apply_save_data()`), sueño = `max_sleep`, vida = `max_health * DEATH_GRACE_HEALTH_RATIO` (0.6). Si el hambre seguía crítica, la vida puede volver a bajar — es a propósito, el desmayo no es un reinicio gratis.
- [x] Al despertar del desmayo, aparecer al lado de la cama (no en la posición del último guardado tal cual) — necesita lógica de colisión/slot para no aparecer superpuesto con otra pieza. **Resuelto:** `save_manager.gd::_respawn_beside_bed()`, llamado tras `load_game()`+`grant_death_grace()` (esperando un frame, para que las piezas viejas `queue_free`-adas ya no colisionen). Busca la cama construida más cercana y prueba 4 lugares pegados a ella (laterales largos primero, después pie/cabecera, en el espacio local de la cama para acompañar su rotación) chocando la cápsula real del jugador contra el mundo — el primero libre gana. Sin cama construida, o con los 4 lados bloqueados, despierta donde cayó (sin castigo extra). Aparece 0.2m arriba del punto para caer apoyada en el piso en vez de nacer incrustada.

### Inventario / Marketplace
- [x] La cama, al comprarse en el marketplace, tiene que aparecer en el inventario — hoy aparece directo desbloqueada en el catálogo de construcción (mismo patrón que el cajón de madera).
- [x] El escritorio debe comportarse igual que la cama: comprarse en el marketplace (precio $0 por ahora) y aparecer en el inventario, no directo en el catálogo de construcción.
- [x] Cama y escritorio (y en general cualquier "furniture" colocable en el mundo) tienen que activar el modo construcción al equiparse desde el inventario, para poder posicionarlos en el mundo como cualquier otra pieza del catálogo.
  - **Resuelto (07/07/2026):** cama/escritorio/cajón de madera pasaron de "requires_item" (desbloqueo permanente del catálogo) a "inventory_item" (ítem físico, `grants_item` en `marketplace_ui.gd`) — se compran cuantas veces se quiera, se suman al inventario, y quedan **ocultas del catálogo de `G`** (`hidden_from_catalog`). `build_system.gd` escucha `Hotbar.inventory_changed`: seleccionar uno de estos ítems en el hotbar activa el modo construcción para esa pieza (`_on_hotbar_changed()`), y colocarla con éxito gasta 1 del stack (`_place_piece()`), igual que cualquier consumible (seed/watering_can). Si el stack llega a 0, se sale del modo construcción solo. No se tocó el flujo de pared/piso/techo/puerta (siguen exactamente igual, vía catálogo de `G`).
  - **Bug encontrado y resuelto en la misma sesión:** comprar entraba en modo construcción sola, sin equipar nada a propósito. Causa: `Backpack.add_item()` intenta meter todo ítem nuevo en el hotbar primero, y si caía justo en el slot ya seleccionado, `_on_hotbar_changed()` lo interpretaba como "la jugadora lo equipó". **Fix:** nuevo `Backpack.add_item_no_hotbar()`, usado por cama/escritorio/cajón (`"skip_hotbar": true` en `marketplace_ui.gd`) — comprarlas ahora solo las suma a la mochila; recién se equipan (y activan modo construcción) cuando la jugadora las arrastra al hotbar y las selecciona a mano.

### HUD / UI
- [x] Prompt de tecla de interacción: reemplazar el rectángulo dibujado a mano por el set de imágenes `assets/hud/keyset` (una imagen por tecla, no un `Label` de texto compuesto a mano). El texto de la acción (ej. "Recolectar") se mantiene como texto, pero con la tipografía de texto general del inventario (Syne Mono — ver `docs/GameDesign/PXD_Diseno_HUD_UI_v1.md` sección 4.1), no la de título (Walter Turncoat). Sin borde negro, todo en blanco. **Resuelto:** `interaction_prompt_3d.gd`/`.tscn` — se sacó el `Sprite3D` del marco (`key_prompt_frame.png`) y el `Label3D` de la tecla; ahora `KeySprite` carga la imagen real del keyset (`assets/hud/keyset/White/`) según la tecla de `InputMap` (letras/números/F1-F12 calzan directo con el nombre de archivo; un diccionario chico cubre las pocas teclas especiales — `Escape`→`esc`, etc.). `ActionLabel` pasó a Syne Mono, `outline_size = 0`. Tamaño del ícono confirmado por la usuaria, quedó bien.
- [x] **Bug encontrado en la misma sesión — el fix de arriba solo se aplicó a cama/escritorio, la huerta se quedó sin ningún prompt.** La usuaria marcó bien el problema de fondo: `bed.gd`/`desk.gd` duplicaban casi el mismo código de proximidad+prompt (copiado, no encapsulado), y la huerta usa un mecanismo totalmente distinto (raycast por frame vía `world.gd`, no proximidad por `Area3D`) que nunca tuvo ningún prompt, ni el rectángulo viejo. **Resuelto en dos partes:** (1) se extrajo `scripts/build/proximity_interactable.gd` — base reusable con Area3D+prompt+guardas de pantallas modales; `bed.gd`/`desk.gd` ahora heredan de ahí y solo implementan `_on_interact()`, `action_text`/`interaction_range` se configuran en la escena. (2) se agregó un prompt compartido en `world.gd` (`crop_prompt`, una sola instancia de `interaction_prompt_3d.tscn` que sigue a la parcela apuntada por raycast cada frame) mostrando "Sembrar"/"Regar" (ícono de click izquierdo, pasado directo ya que esa acción no tiene un nombre en InputMap) o "Cosechar" (tecla E) según el estado de la parcela y la herramienta equipada.
...
## Arquitectura — mejoras acordadas (sesión 2026-07-07) — TODAS IMPLEMENTADAS
Salidas de la revisión de arquitectura pedida por la usuaria. Detalle completo de cada una en [docs/ESTANDARES_TECNICOS.md](docs/ESTANDARES_TECNICOS.md) (secciones "UIState", "Guardado versionado", "Simulación vs. presentación").

- [x] **`UIState` (autoload): una sola fuente de verdad de "qué pantalla está abierta".** Las ~10 cadenas `work_system.is_working or phone_system.is_open or ...` se reemplazaron por `UIState.is_any_modal_open()`; cada pantalla registra su `MODAL_ID` al abrir/cerrar (`phone`/`inventory`/`pause`/`work`/`build_catalog`). Cambio de comportamiento consciente: con el catálogo de construcción abierto ya no se puede caminar (antes sí, por inconsistencia de guardas manuales).
- [x] Contrato de guardado estándar en TODOS los sistemas (`get_save_data()`/`apply_save_data()`) + **save versionado**: formato v2 (un dict por sistema), los guardados v1 existentes se migran solos al cargar (`_migrate_v1()`), sin perder progreso. `SaveManager` quedó como registro (`_save_systems()`) — un sistema persistible nuevo es una entrada ahí.
- [x] Borrada la grilla de cultivo legacy de `world.gd` (~130 líneas del sistema pre-`CropManager` que solo se activaba con un flag de debug) — `world.gd` quedó solo con arranque de partida + interacción de huerta.
- [x] `print()` de gameplay detrás del modo desarrollador: `DevMode.debug_log(tag, msg)` — con `F1` apagado, la consola queda limpia (los `push_error` se muestran siempre).
- [x] Separación simulación/presentación en el marketplace: catálogo y compra viven en `Economy` (`SHOP_CATALOG` + `buy(item_id)`); `marketplace_ui.gd` solo pinta y llama. La app de marketplace del celular diegético va a reusar el mismo servicio. De paso: el hint de cierre de la mochila mostraba el ícono ESC — corregido a Q (`close_window`).
