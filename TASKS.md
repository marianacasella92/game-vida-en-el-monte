...
- [x] (parcial) Implementar el efecto de post-procesado ya documentado en `docs/GameDesign/PXD_Diseno_HUD_UI_v1.md` sección 2.4 (viñeta oscura + desenfoque radial en los bordes de pantalla + audio de respiración pesada) cuando la vida está bajando. **Resuelto (visual):** `scenes/ui/low_health_vignette.gdshader` (mismo patrón 9-tap ya probado en Compatibility que `background_blur.gdshader`, pero con viñeta radial — centro despejado, bordes oscuros y borrosos) sobre un `ColorRect` en el `Hud` (`LowHealthVignette`, primer hijo para dibujarse detrás de las barras). `hud.gd` maneja la intensidad: 0 con vida ≥50% (`VIGNETTE_HEALTH_THRESHOLD`), crece linealmente hasta 1 al llegar a 0. Se oculta con pantallas modales abiertas (el Hud se dibuja encima de esas capas). **Pendiente: audio de respiración pesada** — falta el archivo de sonido (lo consigue la usuaria, CC0), se conecta cuando esté.
